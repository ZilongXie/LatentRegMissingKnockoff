...
# Sampling the underlying variables and impute the standardized covariate matrix 
XX.sample <- function(XX, X, X.under, theta, beta, Omega, XX.ind, 
                      bin.params=NULL, ord.params=NULL, con.params=NULL, 
                      bin.ind=NULL, ord.ind=NULL, con.ind=NULL, 
                      thresh.bin=NULL, thresh.ord=NULL, ...) {
  N <- dim(X.under)[1]
  p <- dim(X.under)[2]
  XX <- as.matrix(XX)
  args <- list(...)
  ...
  random <- sample(1:p)
  for (j in random) {
    cond.mean <- X.under[, -j] %*% meantrans.mat[, j]
    ...
    if(j %in% bin.ind) {
      ...
      X.under[!miss.mask.j, j] <- truncnorm::rtruncnorm(n = 1,
                                                        a = thresh.bin$lower[!miss.mask.j,loc],
                                                        b = thresh.bin$upper[!miss.mask.j,loc],
    ...
    if(j %in% ord.ind) {
      loc <- which(ord.ind == j)
      X.under[!miss.mask.j, j] <- truncnorm::rtruncnorm(n = 1,
                                                        a = thresh.ord$lower[!miss.mask.j,loc],
                                                        b = thresh.ord$upper[!miss.mask.j,loc],
                                                        mean = cond.mean[!miss.mask.j],
                                                        sd = cond.sd)
  ...
  return(list(X.under = X.under, XX = XX))
}

# Stochastic EM algorithm
StEM.refit <- function(X, X.under, Y, Sigma, a.vec, d1.vec, d2.vec, bin.ind=NULL, ord.ind=NULL, 
                       con.ind=NULL, bin.params=NULL, ord.params=NULL, con.params=NULL, 
                       eps=1e-2, max_iter=1000, burn_in=500, is.beta.trace=F, 
                       is.print.iter=F, ...) {
  N <- dim(X)[1]
  p <- dim(X)[2]
  J <- dim(Y)[2]
  ...
  if (!is.null(args$theta0)) {
    ...
    if (length(poly.ind) > 0) {
      # TODO: To be extended to polytomous items with more than 3 categories
  ...
  # Initialize parameters theta and its parameters
  theta <- c(theta0)
  ...
  # Transformation matrix that transforms X_{-i} to the conditional mean;
  ...
  for(iter in 1:max_iter) {
    ...
    res.sample <- XX.sample(XX, X, X.under, theta, beta, Omega, 
                            XX.ind, bin.params, ord.params, con.params, bin.ind, 
                            ord.ind, con.ind, thresh.bin, thresh.ord, 
  ...
  result$theta <- theta
  result$theta.interp <- theta.interp
  result$theta.var <- theta.var
  result$XX <- XX
