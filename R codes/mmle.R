...
XX.sample <- function(XX, X, X.under, X.knock.under, theta, beta, 
                      Omega.aug, XX.ind, bin.params=NULL, ord.params=NULL, 
  ...
  if (is.null(args$meantrans.mat)) {
    matrix(0, nrow = 2*p-1 , ncol = 2*p)
    for (i in 1:(2*p)) {
      meantrans.mat[, i] <- -Omega.aug[i, -i]/Omega.aug[i, i]
    }
  } else {
  ...
  return(list(X.under = X.under,
              X.knock.under = X.knock.under,
              XX = XX))
}

# Stochastic EM algorithm
StEM <- function(X, X.under, X.knock, X.knock.under, Y, Sigma, S, a.vec, d1.vec, d2.vec,
  ...
  # Note that there are 2*p columns
  meantrans.mat <- matrix(0, nrow = 2*p-1 , ncol = 2*p)
  for (i in 1:(2*p)) {
    meantrans.mat[, i] <- -Omega.aug[i, -i]/Omega.aug[i, i]
  }
  ...
  for(iter in 1:max_iter) {
    ...
    res.sample <- XX.sample(XX, X, X.under, X.knock.under, theta, beta, Omega.aug, 
